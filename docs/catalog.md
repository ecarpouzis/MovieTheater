...
own totals (Eric named them) — or, under the **Lists for** switcher (2026-09-04, friends' marks:
`?for=<username>`), a FRIEND's totals, with Rate Movies removed (a rating is the viewer's own). Those
rows are the ONE door onto the lists: the rail parses `my=` and shows the chip but draws no "My lists"
section (`flagsRail: false` — a second door onto the same lists was the duplicate-options bug, Eric
2026-09-04), and the active row pressed again clears it. Photos' index is the album's ways in, not a copy of its tabs. Music's
...
|---|---|---|---|---|
| Movies/TV | Type · Genre · MPA (one pill row, five stops, `stops: true`) · **Years** · Franchise · People (typeahead) · Mood · Subgenre · Era · Theme · Setting | Years (two-thumb, `after: "mpa"`) · Seen/Want/Rated (`my=`, chips only — `flagsRail: false`; whose lists = the URL's `for=<username>`, resolved server-side to the list OWNER on `/API/Browse`, `/API/BrowseLetters`, `/API/BrowseGroups`, `/API/BrowseGroupLetters` and carried in the cache key) | `/API/BrowseFacets` (keyed on the Type scope) + `/API/BrowsePeople` for the person tail | Lists for · Seen · Want to watch · Rate movies · Playlists |
...
- **`clientFacets.ts`** — the client twin of the server's `BrowseFilter` for sections whose list is
  ...
  `q` substring; year range; the RATING FLOOR; the section's flag tests) and
  `countClientFacets(items, extractors)` (the rail's option rows, most-common first). The S2c
  sections mount their specs over it. **The rating floor (`r=`) drops an item with NO score**, the
...
|---|---|---|---|---|---|
| Movies (dense) | `createMoviesListSource` over `clientSource.ts` | Seen · Want to watch (the viewer's own, or a friend's under `for=` — `hooks/useUserLists`) · the back-nav restore · a one-shot browse — the rows the page already holds. Flat views only (an id list has no server grouping); the list is filled in bounded chunks and an untoggle of the list's OWNER edits the array + bumps `dataVersion` | slices | — | — |
...
| Music | `musicSource.ts` over `clientSource.ts` | the rail URL (`q/f/x/y/r`) applied IN MEMORY over the shelf the URL names (`f=kind:` is a SCOPE — the shelf is fetched, never filtered down; `musicFacetSpec.ts` over the shared `useMusicShelf` React-Query resource; `f=artist:`/`f=genre:`/`f=tag:`/`y=`/`r=` filter it, `q` also drives the server song search); pre-S2c `?kind=`/`?tab=` links rewritten once | slices (the catalog sorts, incl. **Top rated** on the server-computed blend and **Most played** on the library-wide play count — both on ALBUMS and ARTISTS, one Sort pill for the section) | artist/decade/year/kind/quality tag/**genre**; artists by the decade they became active, and by genre | artists → albums |
...
| Books | `booksSource.ts` over `booksOData.ts` | the rail URL (`q/f/x/y/r/my`) | `/API/Books/odata/catalog` | `/API/Books/browse/groups` — the axes the HOST advertises on `/browse/facets` (`groupAxes`): collection/series/publisher/decade/franchise/writer/artist (+ Items one-per-series). No advertisement = an old host = the five | collection folders (`dir=`) |
...
| Music | `letter` (albums AND artists) | year · quality tag · **genre** (R9 S10, albums AND artists) | **Genre is the one axis here whose shelves OVERLAP on purpose** — an album is several genres at once, so a record stands under every genre it carries; it is ordered by size (the long tail of one-album genres belongs at the bottom, not spread alphabetically through the middle) and an album with no genre is DROPPED rather than pooled under an "Unknown" shelf. `kind` stays — three values, and the one axis naming which SHELF a row came off. The tag VALUE has no brackets: `MusicNaming.ParseAlbumFolder` strips them at ingest, so `… [FLAC]` on disk is `FLAC` here (two brackets become the one comma-joined `"FLAC, EP"`, which is what the rail's Tag facet matches). Brackets are wildcards only in a T-SQL `LIKE` or a PowerShell path. |
...
| Books | — | writer · artist — **the host ADVERTISES its axes; the pill reads them** | R9 S9 finished the host half: `GroupByPattern` knows `author\|artist`; `CreditHeadsAsync` builds the heads from the SAME `CreditKeyCountsAsync` → `WithDisplayNamesAsync` → `Ordered` the FACETS use, so a shelf's count and its chip's count cannot disagree (rule 2), with the normalized name as the KEY and the readable name as the LABEL, and no `take` (a truncated head list would make the letter rail lie); `BandQuery` narrows by EXISTENCE of a matching `ItemCredit` (a join would return the item once per credit row and inflate the band); and `KeyOf` became **`KeysOf`** — the first many-per-item axis the host has, so one issue stands under every writer AND every artist it credits, fed by a per-band `itemId → names` lookup joined against the band query rather than an id IN-list. `/browse/group-letters` rides the heads unchanged. **The R9 closing pass took the durable fix rather than the flag**: `BrowseController.GroupAxes` is now the ONE list (`GroupByPattern` is built from it) and `/browse/facets` returns it as `groupAxes`; `booksGroupsFor(groupAxes)` in `catalog/sources/booksSource.ts` reads it and `CREDIT_AXES` is DELETED. It exists because **a stale host does not 400 on `groupBy=author` — it silently answers with COLLECTIONS**, so the pill could never tell "understood" from "ignored" and had to wait on a hand-flipped constant after each deploy. Absent list ⇒ an OLD host ⇒ fall back to the five every host has always had (never an empty pill); an advertised axis the SPA has no label for is ignored, never drawn as a raw token. A new axis now needs a label and nothing else. `CatalogTests.The_facets_advertise_exactly_the_group_axes_this_host_can_answer` pins that the advertisement cannot lie (every advertised axis answers differently from collection through the WIRE; an unadvertised one does not). |
...
1. **Existing endpoints only.** No section grew an API for its Overview. Movies reads
   ...
   `/odata/Boardgames` its browse reads; Music the shelf + `/API/Music/Capabilities` (its genre / popularity / rating coverage rows are computed from the fields R9 S10 added to the shelf rows — nothing new is fetched).

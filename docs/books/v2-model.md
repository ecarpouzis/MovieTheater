...
| `Series` | Id | DERIVED canonical series (ids preserved) + the series-scoped facts every issue used to carry; rebuilt by books-resolve --series from ComicDetail.ParsedSeriesKey + SeriesKeyLink | `Id INTEGER, ParsedKey TEXT, CanonicalKey TEXT UNIQUE, Name TEXT, DisplayNameOverride TEXT, IssueCount INTEGER, YearStart INTEGER, YearEnd INTEGER, IsOngoing INTEGER, Franchise TEXT, TitleId INTEGER FK SeriesTitle NULL, PublisherId INTEGER NULL, CvVolumeId INTEGER NULL, ExternalWorkId INTEGER NULL, MuSeriesId INTEGER NULL, ResolvedSynopsisSource INTEGER, ResolvedRating INTEGER, ResolvedAt TEXT` | `(Name, Id)`, `(ParsedKey)`, `(Franchise)`, `(ResolvedRating DESC, Id)`, `(TitleId)` |
| `SeriesTitle` | Id | The TITLE a set of runs belongs to - the tier between Franchise and Series. Issue numbers restart per RUN, so Series must stay run-grained (CollectionNode, ReadingOrderEntry and CvVolumeId are all keyed that way); but 'Green Lantern v2 (1960)' and 'v3 (1990)' are the same title and share a publisher's conventions, so judging one teaches the others. This is where facts that belong to the title rather than to any one run are kept. | `Id INTEGER, Key TEXT UNIQUE, Name TEXT, Franchise TEXT NULL, PublisherId INTEGER NULL, YearStart INTEGER NULL, YearEnd INTEGER NULL, RunCount INTEGER, Note TEXT NULL, CreatedAt TEXT NULL` | `UNIQUE (Key)`, `(Franchise)`, `(Name, Id)` |

...
V2 = {
    ...
    "Item": dict(file="hot", pk=["Id"], purpose="One file = one item (ids preserved = Comics.Id). File identity + the browse-hot flags + the materialized Resolved* scalars. THE browse projection joins Item + Series only.",
        ...
        idx=["(Kind, ResolvedSeries, Id)", "(Kind, ResolvedYear DESC, IndexedAt DESC, Id)", "(Kind, ResolvedRating DESC, Id)", "(Kind, IndexedAt DESC, Id)", "(SeriesId, Id)", "(FolderId)", "(TopFolderId)", "(PublisherId)", "(Kind, NormalizedTitle, Id)", "(Kind, ResolvedPublisher, Id)"]),
    ...
    "MuSeriesLink": dict(file="hot", pk=["SeriesId"], purpose="MangaUpdates link (matched AFTER resolution, so keyed by SeriesId; SeriesMergeService re-keys it)",
        cols="SeriesId INTEGER FK Series, MuSeriesId INTEGER NULL, Status INTEGER, Method TEXT, Confidence REAL, MatchedKey TEXT, CreatedAt TEXT"),
    ...
    "CollectionNode": dict(file="hot", pk=["ItemId"], purpose="DERIVED containment (books-containment); ParentItemId -> Item",
        cols="ItemId INTEGER FK Item, SeriesId INTEGER, Level INTEGER, TrackRole INTEGER, SpanStart INTEGER, SpanEnd INTEGER, ContainsCount INTEGER, ParentItemId INTEGER FK Item NULL, SpanSource INTEGER, SpanLabel TEXT", idx=["(SeriesId)", "(ParentItemId)", "(ContainsCount DESC)"]),
    ...
    "CvIssue": dict(file="hot", pk=["Id"], purpose="ComicVine issue (cover dates feed reading order; description is a synopsis source)",
        cols="Id INTEGER, VolumeId INTEGER, Name TEXT, IssueNumber TEXT, CoverDate TEXT, StoreDate TEXT, Deck TEXT, Description TEXT, ImageUrl TEXT, SiteDetailUrl TEXT, FetchedAt TEXT", idx=["(VolumeId)"]),
...
V1 = {
    ...
    "Comics": dict(targets=["Item", "ItemState", "ItemSignature", "ComicEmbedded", "BookDetail", "ItemCredit", "ItemTag", "Rating"], stage="items", cols={
        ...
        "EmbeddedRating": "->ComicEmbedded.Rating", "UserRating": "drop:0 rows in v1; per-item user ratings arrive as Rating(Source=User) in v2",
        # dead
        "MetadataVersion": "drop:scanner-private constant 1", "Imprint": "->ComicEmbedded.Imprint", "GTIN": DEAD, "SeriesGroup": DEAD, "AlternateCount": DEAD, "Translator": DEAD, "StoryArcNumber": DEAD, "MainCharacterOrTeam": "drop:constant ('Superman') and write-only",

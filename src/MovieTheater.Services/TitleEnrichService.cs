        ...
        public async Task<bool> EnrichAsync(int id, bool isSeries, bool force = false)
        {
            try
            {
                ...
                if (isSeries)
                {
                    ...
                    // Gap-fill only: IMDB always wins (the scrape sets RuntimeMinutes from
                    // aboveTheFoldData.runtime.seconds). OMDB backs it up only when IMDB had none.
                ...
                else
                {
                    ...
                    // Gap-fill only: IMDB always wins; OMDB backs up RuntimeMinutes only when IMDB had none.

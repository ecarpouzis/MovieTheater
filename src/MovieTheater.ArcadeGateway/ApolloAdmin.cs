...
public sealed class ApolloAdmin
{
    ...
    /// <summary>The /api/apps payload: host identity + the app list. Apollo assigns each app a
    /// stable <c>uuid</c>, and the host a <c>host_uuid</c>/<c>host_name</c> — exactly the fields an
    /// Artemis <c>.art</c> launch shortcut needs (see <see cref="BuildArtShortcut"/>).</summary>
    public sealed record AppsSnapshot(string? HostUuid, string? HostName, List<JsonObject> Apps);

    public async Task<AppsSnapshot?> GetAppsSnapshotAsync(CancellationToken ct)
    {
        try
        {
            ...
            return new AppsSnapshot(
                (string?)root["host_uuid"],
                (string?)root["host_name"],
                root["apps"]!.AsArray().Select(n => n!.AsObject()).ToList());
        }
        catch (Exception ex)
        {
    ...
    public async Task<List<JsonObject>?> GetAppsAsync(CancellationToken ct) =>
        (await GetAppsSnapshotAsync(ct))?.Apps;

    /// <summary>
    /// An Artemis launch shortcut (<c>.art</c>): tapping it on a paired Android device jumps
    /// STRAIGHT into streaming this app — no navigating the client's grid. Format verified against
    /// Artemis' own ShortcutTrampoline/ShortcutHelper source (moonlight-noir branch): line-based
    /// <c>[key] value</c>, host_uuid + host_name + app_uuid/app_name. The device must already be
    /// paired — the trampoline verifies the host is known before launching.
    /// </summary>
    public static string BuildArtShortcut(string hostUuid, string hostName, string appUuid, string appName) =>
        "# Artemis app entry\n" +
        "# Generated by the arcade heavy lane\n" +
        "\n" +
        $"[host_uuid] {hostUuid}\n" +
        $"[host_name] {hostName}\n" +
        $"[app_uuid] {appUuid}\n" +

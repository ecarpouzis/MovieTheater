...
// pill (Movies only), and a single gear that opens a small menu holding User Settings, Library Review
// (editors) and User Administration (admins). Those tools used to be their own always-visible icons,
// which crowded the 200px header and — once the Playlists pill was added — overflowed and hid the
// admin/review buttons entirely. Folding them behind the gear keeps them one click away without a
// dedicated row.
function UserPanelHeader({ userData, setSettingsModalOpen, setAdminModalOpen, onOpenPlaylists }) {
          ...
          <>
            ...
            <div className="user-menu">
              ...
              {userData.canEditMovies && (
                <button className="user-menu-item" onClick={() => { close(); history.push("/review-ingest"); }}>
                  Library Review
                </button>
              )}
              {userData.isAdmin && (
                <button className="user-menu-item" onClick={() => { close(); setAdminModalOpen(true); }}>
                  User Administration
                </button>
              )}
            </div>

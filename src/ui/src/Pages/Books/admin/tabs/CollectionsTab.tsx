...
function Tile({ id, name, count, epoch }: { id: number; name: string; count: number; epoch: number }) {
  ...
  return (
    <div className="adm-tile">
      <button type="button" className="adm-tile-art" onClick={() => input.current?.click()} disabled={upload.isPending} title="Upload a new icon">
        {src && !missing ? <img key={`${nonce}:${epoch}`} src={`${src}${src.includes("?") ? "&" : "?"}v=${nonce}`} alt="" onError={() => setMissing(true)} /> : <span className="adm-tile-empty">no icon</span>}
        <span className="adm-tile-hover">{upload.isPending ? "Uploading…" : "Upload"}</span>
      </button>
      ...
      <div className="adm-tile-name" title={name}>{name}</div>
      <div className="adm-tile-sub">{count.toLocaleString()} comics · <code>#{id}</code></div>
  ...
  return (
    <div className="adm-tab">
      <section className="adm-card">
        <header className="adm-card-head"><div className="adm-card-text"><h3 className="adm-card-title">Collection icons</h3><p className="adm-card-desc">Click a tile to upload its icon (JPG/PNG/WebP, up to 4 MB). Icons are hand-made art: a cache clear never touches them.</p></div></header>
        {facets.isLoading ? <div className="adm-muted">Loading…</div> : collections.length === 0 ? <div className="adm-muted">No collections yet.</div> : (
          <div className="adm-tiles">{collections.map((c) => <Tile key={c.id} id={c.id} name={c.name} count={c.count} epoch={epoch} />)}</div>
        )}
      </section>
    </div>
  );

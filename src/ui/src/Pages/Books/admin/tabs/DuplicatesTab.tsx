        ...
        {g.members.map((m) => (
          <label key={m.itemId} className="adm-member">
            ...
            <div className="adm-member-text">
              ...
              <div className="adm-muted"><code>{m.path}</code></div>
              <div className="adm-muted">{m.pageCount ?? "?"} pp · {fmtSize(m.fileSize)} · <code>#{m.itemId}</code></div>
            </div>
          </label>
        ))}
      </Radio.Group>
    ...
    <div className="adm-tab">
      ...
      <section className="adm-card">
        <header className="adm-card-head">
          <div className="adm-card-text"><h3 className="adm-card-title">Groups</h3><p className="adm-card-desc">Pick the keeper, then hide the rest. A hidden duplicate leaves every browse surface but stays in the Directory, dimmed, because the file really lives there.</p></div>
          <Space wrap>
            <Segmented options={["Pending", "Resolved"]} value={state} onChange={(v) => { setState(String(v)); setPage(0); }} />
            <span className="adm-muted">{total.toLocaleString()} groups</span>
        ...
        {groups.isLoading && <div className="adm-muted">Loading…</div>}
        {!groups.isLoading && total === 0 && <div className="adm-muted">No {state.toLowerCase()} groups.</div>}

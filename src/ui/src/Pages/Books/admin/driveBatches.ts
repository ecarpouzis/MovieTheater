/**
 * The client-driven loop for a WHOLE list the host only serves in pages (every decision, every broken
 * file, every dedup group): the caller's `step` fetches one page and reports what it got; this
 * accumulates, reports progress, honours an abort, and BREAKS on no progress — a page that returns
 * nothing new, or the same cursor twice — so a host that keeps answering the same page can never
 * spin the browser forever. The house rule for bulk work, on the client side.
 */

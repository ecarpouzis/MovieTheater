/**
 * The Books section's own skin, layered under the site's theme: seven backdrops (the standalone's nine
 * minus the two retired ones) and four display-type themes, both offered as tweak rows and applied to
 * the section root as `data-books-backdrop` + inline `--books-*` custom properties. MT's `data-theme`
 * remains the light/dark authority: a backdrop belongs to one family, and a remembered value from the
 * other family falls back to that family's default.
 *
...
export function applyBooksTheme(root: HTMLElement | null, extras: Record<string, string> | undefined, theme: SiteTheme): void {
  if (!root) return;
  const backdrop = resolveBackdrop(extras?.[BACKDROP_EXTRA], theme);
...
export function booksThemeStyle(extras: Record<string, string> | undefined, theme: SiteTheme): Record<string, string> {
  const backdrop = resolveBackdrop(extras?.[BACKDROP_EXTRA], theme);
  const font = BOOKS_DISPLAY_FONTS[resolveDisplayFont(extras?.[DISPLAY_EXTRA])];
  const t = BOOKS_BACKDROPS[backdrop];
  return {

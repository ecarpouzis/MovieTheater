...
export default function GameModal({ game, onClose, onStart, onManageSaves, creating, canEditMovies, renderers = [], initialVersionId = null }) {
  ...
  // No ROM prewarm on modal open (arcade perf program P7, 2026-09-05, withdrawn the same night): staging a
  // JIT title the moment its card is on screen did cut a cold PS2 start from 40.7 s to 3.9 s, but Eric
  // browses many cards without ever pressing Play, and every one of those would have extracted a multi-GB
  // ISO into the ROM cache (disk churn + evictions of titles people DO play). The gateway/site endpoint
  // (`POST /API/Arcade/Game/{id}/Prewarm`) stays for a trigger that means intent — nothing calls it today.
